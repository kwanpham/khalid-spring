pipeline {
    ...
    environment {
        MYSQL_ROOT_LOGIN = credentials('mariadb-root-login')
    }
    stages {
        ...
        stage('Packaging/Pushing imagae') {
            ...
            steps {
                withDockerRegistry(credentialsId: 'dockerhub-quanph', url: 'https://index.docker.io/v1/') {
                    sh 'docker build -t chomchom9606/springboot .'
                    sh 'docker push chomchom9606/springboot'
                }
            }
        }
        ...
        stage('Deploy MySQL to DEV') {
            steps {
                ...
                sh 'sleep 20'
                sh "docker exec -i mairadb mariadb --user=root --password=${MYSQL_ROOT_LOGIN_PSW} < script"
            }
        }
        ...
        stage('Deploy Spring Boot to DEV') {
            steps {
                ...
                sh 'docker container run -d --rm --name khalid-springboot -p 8081:8080 --network dev chomchom9606/springboot'
            }
        }
